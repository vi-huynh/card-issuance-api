...
RSpec.describe V1::Admin::Brands::IndexService do
  ...
  describe 'listing brands' do
    ...
    it 'returns every persisted brand' do
      expect(result.success?).to eq(true)
      expect(result.data[:items]).to match_array(brands)
    end
  ...
  describe 'pagination' do
    let!(:brands) { create_list(:brand, 15) }
  ...
  describe 'sorting' do
    ...
    context 'sorted by name ascending' do
      ...
      it 'returns brands ordered alphabetically' do
        expect(result.data[:items]).to eq([ brand_a, brand_b, brand_c ])
      end
    end
    ...
    context 'sorted by name descending' do
      ...
      it 'returns brands in reverse alphabetical order' do
        expect(result.data[:items]).to eq([ brand_c, brand_b, brand_a ])
      end
    end
  end

  describe 'when there are no brands' do
    let(:params) { {} }
