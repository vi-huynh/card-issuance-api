...
RSpec.describe V1::Clients::Products::IndexService do
  ...
  let(:brand_a) { create(:brand, name: 'Alpha Brand') }
  let(:brand_b) { create(:brand, name: 'Beta Brand') }

  describe 'when the client has access to products across multiple brands' do
    let!(:product_a) { create(:product, brand: brand_a, name: 'Gift Card 10', price: 10, status: :active) }
    let!(:product_b) { create(:product, brand: brand_b, name: 'Voucher 20', price: 20, status: :active) }
    let(:params) { {} }
  ...
  describe 'when filtering by price range' do
    let!(:product_low) { create(:product, brand: brand_a, name: 'Low', price: 5, status: :active) }
    let!(:product_mid) { create(:product, brand: brand_a, name: 'Mid', price: 20, status: :active) }
    let!(:product_high) { create(:product, brand: brand_a, name: 'High', price: 50, status: :active) }
  ...
  describe 'when filtering by product name' do
    let!(:matching) { create(:product, brand: brand_a, name: 'Special Offer', price: 10, status: :active) }
    let!(:non_matching) { create(:product, brand: brand_a, name: 'Standard', price: 10, status: :active) }
    let(:params) { { name: 'special' } }
  ...
  describe 'when a granted product is inactive' do
    let!(:inactive_product) { create(:product, brand: brand_a, name: 'Gone', price: 10, status: :inactive) }
    let(:params) { {} }
  ...
  describe 'when a granted product belongs to another client' do
    ...
    let!(:product) { create(:product, brand: brand_a, name: 'Shared catalog product', price: 10, status: :active) }
    let(:params) { {} }
