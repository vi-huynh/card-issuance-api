  ...
  module Admin
    module Reports
      class ShowService < ApplicationService
        def initialize(params: {}, connection: ActiveRecord::Base.connection)
          super(params:)
        ...
        def sanitized_sql
          sql = +<<~SQL
            ...
            FROM cards
          SQL
          sql << "INNER JOIN products ON products.id = cards.product_id\n" if @params[:brand_id].present?
          sql << "WHERE cards.created_at BETWEEN :date_from AND :date_to\n"
          sql << "AND products.brand_id = :brand_id\n" if @params[:brand_id].present?
          sql << "AND cards.client_id = :client_id\n" if @params[:client_id].present?
